...
> 最新版本 **0.7.1(2026-09-22)** 已正式发布；推送版本标签后由 GitHub Actions 构建并附加发行 jar。
...
- **"与上次启动对比"永久退化为"首次记录"（真机验收发现）**：启动画像的反射反序列化要求装箱类型精确匹配，而 JSON 里的小整数被解析成 `Integer`——绑到 `StartupProfile` 的装箱 `Long` 字段（`premainNanos` / `sampleIntervalMs`）即抛异常，整份文档反序列化失败，于是每次启动记一条 WARN 并按"无基线"处理（该路径自 v0.2.0 的不可变模型重构起即失效，v0.2.0 验收当时走的是内存画像路径）。修复：`Json` 新增 `decodeLenient`——严格绑定失败时，仅把**目标类里声明为装箱 `Long` 的字段**在 JSON 树中由小整数归一化为 `Long` 后重试一次（不动基本类型与其它装箱类型，避开把本来能绑的 `int` 字段弄坏），`api` 公开契约不变；`LocalFileMetricStore` 的两处画像读取改走该入口。真机复验：同一起始条件（`sampleIntervalMs=10`）下读取失败告警由 1 条降为 0 条，启动对比恢复为"总时长 25.6s → 26.9s（+1.3s, +5.1%）"。
...
- **FR-28 的 BungeeCord 日志检索在真机上恒降级（真机验收推翻）**：实现按 spec 假设解析工作目录根下的 `proxy.log`，但 **BungeeCord #2088 + JDK 21 真机（全新工作目录）实测不存在该文件**——当前日志是带代次后缀的 `proxy.log.0`（同目录另有 `proxy.log.0.lck` 锁文件；重启后沿用同一文件追加），于是 MCP `log_tail`/`log_search` 在 BungeeCord 上恒返回 `{"available":false,"reason":"日志文件不存在"}`（降级本身是结构化的、不报错，故此前未暴露）。修复：解析改为"优先明文 `proxy.log`，否则取按修改时间最新的 `proxy.log.<纯数字>`"（按时间而非代次编号排序，避开 JDK `FileHandler` 代次语义差异，并天然排除 `.lck`/`.gz`），真机落点同时写入类 KDoc 与 spec；Velocity 侧（`logs/latest.log`）真机通过、未受影响。
- **MCP 控制面桥护栏与写超时（技术债 #26）**：桥入站帧/消息加 1 MiB 上限（防单帧 OOM，分片累积由 `ArrayList<Byte>` 换 `ByteArrayOutputStream`）；桥写由阻塞锁改 `ReentrantLock.tryLock` 非阻塞 + 临时收紧 `soTimeout` 兑现"绝不阻塞"承诺，调用方按返回值记丢弃日志（业务事件带 dedupKey）；`/probe flamegraph` 落盘转 `submitAsync` 并补失败提示键；重连退避加 ±20% 抖动（防雪崩重连）。
- **IoC 扫描失败防护与 plugin 模块补测试（技术债 #26）**：`getRunningClassesInJar` 三段前置反射（ProjectScanner 类 / getRunningClassMapInJar / toClass）与容器/条件评估器获取全部纳入 `runCatching`，失败显式告警并跳过——不再出现"注入链整体中断且无任何症状"；候选 >0 但注册 0 时同样升级告警；plugin 模块新建 test 源集（7 例）。另把插桩安全表述对齐现实：`SECURITY.md` / `testing-and-quality.md` 改为真实防线（`FrameSafeClassWriter` + transform 失败回退 + 多版本真机矩阵），`asm-util` 的 `CheckClassAdapter` 从未在依赖与代码中出现。
- **FR-25 `/metrics` 逐插件取值与 `/probe startup` 不同源（真机验收发现）**：画像里逐插件耗时两套来源并存（agent/Incision 插桩实测 vs `logs/latest.log` 时间差解析近似），命令按"择优"取前者、指标直接取后者，于是挂 agent 时同一实例上面板显示 1.0s（近似）而命令显示 0.9s（实测 868ms）；且近似榜未截断，会把每个插件都写成一条 label。修复：抽出共用的 `SlowPluginRanking`（择优顺序 Incision > agent > 日志解析，并按 `startup.top-n` 截断），命令与指标统一走它；`format(...)` 新增 `startupTopN` 参数由 `MetricsHttpHandler` 传配置值。真机复验：`serverprobe_startup_plugin_seconds{plugin="ServerProbe"} 0.868` 与 `/probe startup` 的 `ServerProbe 0.9s` 同源一致。
