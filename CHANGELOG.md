...
- **背包业务 Provider(对接 AllinInventorySync,JBIS,对接 JM FR-125)**:platform-bukkit 新增 `InventoryProvider`(`inventory` 域),`compileOnly` AllinInventorySync 自包含 api(其 ADR-0014 把对外 DTO 归并入 api 模块、单制品对外可消费;运行期由目标服务端提供),经 `AllinInventorySyncProvider.isAvailable/get` 发现 + 降级。只读 `view`(`getPlayerInventory(uuid)` 回源含离线 → 结构化视图:背包 / 末影箱物品数组 + 基础属性 + online + dataVersion,玩家无数据回 `exists=false`)+ 写 `writeInventory`/`writeEnderChest`/`writeBasicAttrs`(经写门面 `getInventoryWriteApi()`,落盘回执 `WriteResult` 透传 success/online/newDataVersion/errorCode)。守写契约:幂等键 `taskId`(CP 注入)→ `InventoryWriteDto.requestId` 持久去重(缺则拒绝防刷物品),`base + edited` delta 透传,operator 透传(空回退 `JianManager`);物品过桥见 [ADR-0016](docs/adr/0016-inventory-business-item-transport.md)(读富、写以 `nbtBase64` 为准、`base/edited` 经 JSON 门面 `getStringList` 物品串列表 / 属性对象承载,`player` 为 UUID);写门面 future 有界阻塞(短于派发超时,超时优雅降级)。`@Service` + `@PlatformSide(BUKKIT)` + `@PostConstruct` 自注册到 `BusinessHost`;纯解析 / 校验 / 编码逻辑抽 `InventoryEnvelope`(可单测,encode 返纯 Map)。`./gradlew build` 全模块编译 + IOC(beans=24,errors=0)+ detekt + 单测(manifest / 读写校验 / 物品 · 属性解码 / 视图 · 物品 · 回执编码,10 例)全绿;**真机(真 AllinInventorySync 服读写真实背包、回执正确)待端到端复验**。
