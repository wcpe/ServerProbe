...
> ⚠️ **M1 已实现:`./gradlew build` 编译 + 单元测试通过,真机验证进行中;尚未发布、未经生产验证。**
>
> M1(P1–P9)代码已落地——多平台骨架、启动剖析、JVM/服务器指标、`/probe` 命令、本地文件落盘、代理端基础。功能标注:✅ 已实现(待真机验证) / 🚧 规划中(M2–M4)。完整路线见 [CHANGELOG](CHANGELOG.md) 与 [PRD](docs/PRD.md)。
...
- **启动性能剖析** ✅ 已实现(待真机) —— 端到端启动总时长(`ServerLoadEvent` − JVM 启动时刻)、逐插件启用间隔耗时、生命周期分段(CONST/INIT/LOAD/ENABLE/ACTIVE)耗时,并与上次对比。(逐世界加载耗时、逐项 Δ 明细留 M2)
- **运维指标采集** ——
  - **JVM** ✅ 已实现(待真机):堆 / 非堆内存、各内存池、GC(明细 + young/old)、线程数 / 死锁、类加载、进程 & 系统 CPU、uptime、启动参数(全版本 + 全平台通用)。
  - **服务器(Bukkit)** ✅ 已实现(待真机):TPS(1/5/15min)、MSPT(均值 + p95/p99)、在线人数、运行时长;多版本兼容(Paper API / 低版本 NMS / 自采样),Folia 全局 N/A。
  - **世界(Bukkit)** 🚧 M2:按世界的区块 / 实体 / 方块实体(Folia 走 `callRegion{}`)。
  - **代理端(BungeeCord)**:总在线、各子服在线 ✅ 已实现(待真机);子服 ping / 可达性、玩家路由 🚧 M2。
- **多通道呈现** ——
  - 游戏内命令 `/probe …`(权限受控)✅ 已实现(待真机)
  - 本地文件落盘(启动画像 JSON)+ 与上次对比 ✅ 已实现(待真机)
  - 开放接口(只读 API)✅ 已实现(待真机)
  - Prometheus `/metrics` + Grafana 看板 🚧 规划中(M2)
  - Web 面板(启动画像详情、历史趋势)🚧 规划中(M3)
- **全版本 + 多平台** ✅ 已实现(待真机) —— 单 jar 运行于 Bukkit 系 1.8 – 1.21.11(含 Folia)+ BungeeCord,核心 Java 8 字节码通用,版本 / 平台差异以最小胶水隔离。
...
> ✅ 六命令 M1 已实现(编译 + 单测通过,待真机验证);`world` 为占位、`proxy` 为基础(完整版 M2)。语法与输出以最终实现为准。

| 命令 | 作用 | 状态 |
|---|---|---|
| `/probe health` | 服务器健康总览(TPS/MSPT/内存/CPU/在线) | ✅ 已实现(待真机) |
| `/probe startup` | 启动画像:总时长、慢插件 Top-N、各世界耗时、与上次对比 | ✅ 已实现(待真机) |
| `/probe tps` | TPS(1/5/15min)与 MSPT(均值 + p95/p99) | ✅ 已实现(待真机) |
| `/probe gc` | GC(young/old)、堆 / 非堆 / 内存池 | ✅ 已实现(待真机) |
| `/probe world` | 各世界区块数、实体数、方块实体数 | 🚧 占位(M2) |
| `/probe proxy` | 代理端:总在线、各子服在线(ping/路由 M2) | ✅ 基础(待真机) |
