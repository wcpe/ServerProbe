...
| (可选,M5)主线程栈采样 | premain agent 按线程名 `"Server thread"` 周期抓栈 | 启动期"无日志卡顿"热点 |
...
| ADR-11 | 为补 ServerProbe 加载前盲区,做**可选的 premain Java Agent** 增强(命令行 `-javaagent`,默认不启用) | 命令行 premain 在启动期加载,**不是被 ADR-1 否决的运行时 self-attach**,不受 JEP 451 限制(Paper+JDK21/24 零警告);唯一能拿到本插件 onEnable 之前的逐插件精确耗时/库下载/主线程栈 | 仅靠日志解析(秒级、拿不到无日志卡顿);运行时 self-attach(Paper/JDK21+ 默认禁用,已被 ADR-1 否决) |
