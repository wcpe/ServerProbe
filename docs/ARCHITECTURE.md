...
| ADR-8 | **运行期** CPU 热点/火焰图不自研,建议并用 spark;**启动期(premain 窗口)火焰图自研**(M5) | spark 成熟、社区标准,运行期不重复造轮子;但 spark 难介入 premain 阶段,而"启动期"恰是本项目首要场景,故自研一个轻量、启动期专用的折叠栈火焰图(`/probe flamegraph`),与运行期并用 spark 互补不冲突 | 运行期也自研(重复造轮子);启动期也外包(spark 覆盖不到 premain) |
...
| ADR-11 | 为补 ServerProbe 加载前盲区,做**可选的 premain Java Agent** 增强(命令行 `-javaagent`,默认不启用);采集逐插件/库下载/世界/配置/事件/命令耗时 + 多线程折叠栈,并据此导出启动火焰图 + 嵌套时间线 | 命令行 premain 在启动期加载,**不是被 ADR-1 否决的运行时 self-attach**,不受 JEP 451 限制(Paper+JDK21/24 零警告);唯一能拿到本插件 onEnable 之前的精确耗时与调用栈;折叠栈保留调用层级以还原真火焰图;采集收敛在启动窗口、失败静默降级,不成事故源 | 仅靠日志解析(秒级、拿不到无日志卡顿);运行时 self-attach(Paper/JDK21+ 默认禁用,已被 ADR-1 否决);逐帧词频(无法还原调用层级) |
| ADR-12 | **HTTP/TCP 外呼监控**插桩 JDK 层(`HttpURLConnection.getInputStream` + `Socket.connect`),而非逐个 HTTP 客户端库 | JDK 层是收口点:一处覆盖绝大多数(含 TabooLib 下载、HTTPS),Socket 兜底非 HttpURLConnection 客户端;插桩极简(仅 `this`+起始时刻交给数据桥)、逻辑全程 try/catch、经 ASM CheckClassAdapter 对真实 JDK 类校验合法,确保绝不破坏 JVM 网络;敏感头/参数 agent 侧脱敏;有界缓冲防泄漏 | 逐个 hook OkHttp/Apache 等(各插件重定位、维护面大、覆盖不全);代理/URLStreamHandler(无法归因调用方、漏 startup) |
