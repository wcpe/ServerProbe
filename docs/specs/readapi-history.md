...
- 真机 E2E（read-api 场景扩展）：第三方夹具写入历史 → 经 `ServerProbeApi` 读回数据一致；主线程调用该方法的用例**不进** E2E（红线：文档明示异步调用即可）。**已实现并真机通过**（2026-09-22，Paper 1.20.1）：夹具在**异步线程**调用 `historySnapshots`，并与**落盘文件逐条对照**（由新到旧、条数上限、每个时间戳必须能在文件里找到，且要求落盘至少两份以免"顺序"成为空校验）——只验非空无法区分内存回读与磁盘回读，而本场景要验的正是落盘侧；结果文件记 `historyCount` / `historyFileSnapshots` / `historyMatched` / `historyNewestAt`。
