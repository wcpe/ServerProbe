...
- MCP 新增工具 `log_tail`：入参 `lines`（默认 200，上限 256）与可选 `keyword`，返回 `logs/latest.log` 末尾 N 行；关键字过滤时返回命中行（含行号偏移）。
- MCP 新增工具 `log_search`：入参 `keyword`、`sinceOffset`（起始字节偏移，默认 0）、可选 `maxLines`（默认 100，上限 256），返回自偏移起的命中行与下一偏移（游标分页）。
  - 上限 256 = 响应写入器条目上限（`McpJsonWriter.MAX_ITEMS`）：写入器对任意数组一律截断到该值且不产生标记，工具若声称更大上限即为"静默少给"（`log_search` 还会让 `nextOffset` 越过被丢弃的命中行，后续分页再也取不回）。两者钳到同一上限后恒不触发截断。
