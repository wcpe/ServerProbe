...
class VelocityFr13MatrixContractTest {
    ...
    fun `FR13 声明三个 Velocity 版本的独立双 Paper 矩阵`() {
        ...
        assertTrue(build.contains("backend(\"paper-fr13-v31-a\") {\n        platform = paper\n        version = \"1.18.1\""))
        assertTrue(build.contains("backend(\"paper-fr13-v31-b\") {\n        platform = paper\n        version = \"1.18.1\""))
    }

    @Test
    fun `FR13 代理验收桩同时验证 RTT 路由和两名玩家延迟`() {
        val harness = File(
            "../../e2e/harness-velocity-matrix/src/main/java/top/wcpe/mc/plugin/serverprobe/e2e/velocity/VelocityFr13MatrixHarness.java",
        ).readText()
    ...
    fun `FR13 协议机器人只有切服角色发送服务器切换命令`() {
        val driver = File("../../e2e-bot/src/fr13VelocityMatrix.js").readText()
    ...
    private fun assertMatrix(build: String, suffix: String, version: String, javaVersion: Int?) {
        assertTrue(build.contains("backend(\"paper-fr13-$suffix-a\")"))
        assertTrue(build.contains("backend(\"paper-fr13-$suffix-b\")"))
        assertTrue(build.contains("proxy(\"velocity-fr13-$suffix\")"))
        assertTrue(build.contains("version = \"$version\""))
        assertTrue(build.contains("scenario(\"fr13-velocity-$suffix\")"))
        assertTrue(build.contains("backends(\"paper-fr13-$suffix-a\", \"paper-fr13-$suffix-b\")"))
        assertTrue(build.contains("via = \"velocity-fr13-$suffix\""))
