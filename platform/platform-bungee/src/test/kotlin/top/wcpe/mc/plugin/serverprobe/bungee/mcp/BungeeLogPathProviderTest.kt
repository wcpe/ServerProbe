 ...
 *
 * BungeeCord 根目录 = 进程工作目录、日志为根下 `proxy.log`：以临时目录切工作目录
 * （`user.dir` 可写属性）验证路径解析与字符集供给；文件存在性降级路径由 core 的
 * LogTailToolProvider 统一处理（不在本测试范围）。
 */
class BungeeLogPathProviderTest {

    @Test
    fun `根目录为进程工作目录且日志指向 proxy_log`() {
        ...
        assertEquals(
            expectedRoot.resolve("proxy.log"),
            provider.latestLog(),
            "最新日志应为根下 proxy.log"
        )
    }
