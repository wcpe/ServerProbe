...
进入仓库的一切内容（代码、注释、文档、配置、提交信息、PR 标题与正文）按**永久公开**对待（公开历史不重写，见 §5.4）。**隐私信息、本机绝对路径、禁止入库的文件与内容，一律不得提交**；提交前逐项过本节自检。

### 7.1 隐私与敏感信息

- 凭据密钥类：token、密码、API key、证书/私钥、带凭据的完整 URL（如 Webhook 地址）。仓库内示例/默认配置只放空串或占位值（口径见 `config-files.md` 与 `SECURITY.md`）。
- 个人与基础设施类：本机用户名/账户名、内网地址、服务器 IP/域名、真实网段拓扑。
- 业务数据类：玩家名、聊天内容、日志原文等真实运行数据片段。

### 7.2 机器绝对路径

文档、注释、示例与提交信息中**禁止出现本机绝对路径**（如 `C:\Users\<用户名>\...`、`D:\Projects\...`）：它们泄漏账户名与目录结构，且离开本机即失效。需要指代位置时改用相对路径、工程名或环境描述（如"运行于 JDK 25 的代理进程"）。本仓库曾有真机证据带出机器路径、后经专项提交全仓清理的先例，勿重蹈。

### 7.3 禁止入库的文件与目录

以下类别均已由 `.gitignore` 排除，提交时不得用 `git add -f` 等方式强行加入：

- 易朽过程稿：`.tmp/`（§2.2）；同时**禁止在入库文档中引用 `.tmp/` 内路径**——对读者是无法核验的悬空引用。
- 构建产物与依赖：`build/`、`target/`、`.gradle/`、`.kotlin/`、各类 jar/war/zip（`gradle-wrapper.jar` 除外）。
- 环境与工具会话数据：`.env`、`.idea/`、`.vscode/`、`.workbuddy/`、`.narrafork/`。
- 运行时数据：`run/`、`*.log`、`hs_err_pid*` 崩溃转储。
- 其他大型二进制文件（视频、数据集等）；确属必要的小型资产先经用户确认再入库。

发现新的应排除类别时，先经用户确认补 `.gitignore`，再提交。

### 7.4 提交前自检

1. `git status`：确认没有意外文件混入（临时脚本、日志、本地产物）。
2. 快扫新增内容有无 7.1 / 7.2 项：搜盘符路径（`C:\Users`、`D:\` 等）与 `token` / `password` / `secret` / `webhook` 等关键字（Grep 或在 `git diff` 中检查均可）。
3. 误提交的处理：未 push → 在未共享分支上立即整理（§5.4 允许的范围内）；已 push → 泄露的凭据**立即轮换作废**，文件用新提交移除，禁止 rebase 公开历史（§5.4）。
