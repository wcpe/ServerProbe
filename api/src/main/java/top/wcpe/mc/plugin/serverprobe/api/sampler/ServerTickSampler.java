 ...
 * - Paper:经官方 {@code getTPS()} 等 API;
 * - 老版本/纯 CraftBukkit:经 NMS 反射读取或自建采样;
 * - Folia:无全局值,标 N/A({@link #sample} 各数值字段返回 null)。
 */
public interface ServerTickSampler {

    /**
     * 本采样器的数据来源,供呈现层标注数据背景。
     *
     * 固有属性,不随单次采样结果变化;Folia 实现固定为 {@code UNAVAILABLE};
     * Paper/NMS 实现即使本次取不到值也保持其 source,靠 {@link TickSample} 字段 null 表瞬时缺值。
     *
     * @return 本采样器的数据来源。
     */
    TickSampleSource getSource();
