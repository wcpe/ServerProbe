## 变更说明
<!-- 这次改了什么、为什么。关联需求（PRD FR）或 Issue。 -->

## 类型
- [ ] feat 新功能
- [ ] fix 修复
- [ ] refactor 重构
- [ ] revert 回滚
- [ ] docs / chore / 其他

## 防漂移自检（见 docs/CONTRIBUTING.md 与 .claude/rules/）
- [ ] 方向一致：已读相关 PRD / ARCHITECTURE / ADR，未静默违背既定决策
- [ ] 范围合规：未夹带后续阶段（M3/M4）能力或镀金（scope-discipline）
- [ ] 测试：新增 / 改的行为有测试，`./gradlew build` 全绿（验证门通过）；涉及真机维度的已附该端复验结果
- [ ] 文档同步：受影响的 PRD / ARCHITECTURE / API 已改到一致（doc-sync）
- [ ] 架构决策：如有，已写新 ADR（推翻旧决策则标记取代，不删）
- [ ] CHANGELOG：用户可见变更已记入未发布段
- [ ] 提交规范：中文 Conventional Commits、无 AI 署名
- [ ] 内容卫生：无隐私/凭据、无本机绝对路径、无禁入文件（git-commit §7）

## 破坏性变更 / 迁移
<!-- 如有对外 API / 配置 / 落盘格式（schemaVersion）的破坏性变更，写明影响与迁移步骤；否则填"无"。 -->
